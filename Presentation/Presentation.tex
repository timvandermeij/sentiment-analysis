...
{%
  ...
  \begin{tabular}{@{\,}r <{\hskip 2pt}!{\timeline} >{\raggedright\arraybackslash}p{9.5cm}}
    ...
    % I assume "extraction search time" referred to the problem of how the tar 
    % files were extracted, but that was a bit too specific since we made other 
    % optimizations/clarifications in the preprocessor as well.
...
% Regular slide
% TODO: add why we do not want a regressor here?
% TODO: passive aggressive is better than random forest, so why are the language plots
%       made with a random forest regressor?
\setbeamertemplate{navigation symbols}{}
...
\begin{frame}[fragile]{Experiments and results (continued)}
  \begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{random-forest-plot.pdf}
  \end{figure}
\end{frame}
...
\begin{itemize}
  ...
  \item Bigger potential: can be extended to users, repositories,
        communities, \ldots
