...
\usepackage{hyperref}
\usepackage{csquotes}
\usepackage[backend=bibtex, sorting=none, dateabbrev=false, 
...
% TODO: Explain which kinds of plots the framework can produce and
...
% add details on our final choice.
\ldots

\subsection{Performing sentiment analysis per language}\label{sec:sentiment-analysis-per-language}
% TODO: Explain the experiment, why we want to do this and show and explain
% the generated language plots, much like we have done for the presentation.
\ldots
...
that the passive aggressive classifier is the most accurate on the commit
comments dataset, while other classifiers such as the random forest classifier
also yield high accuracies.
...
This appendix is based on an instruction and readme file that accompanies the 
...
For the default group of \texttt{id} and the group \texttt{score}, running \texttt{preprocess.py} only needs to be done once. If all required data is available, \texttt{preprocess.py} will do nothing.
...
requires the \texttt{2015-01-29} commit comments dump in order to 
...
commit comments dumps, whereas working with the \texttt{2015-01-29} dataset is 
usually enough. The MPI parallelization can be done on one machine using 
\texttt{mpirun -n \textless{}num\_processes\textgreater{} python preprocess.py 
...
\texttt{pympi \textless{}num\_processes\textgreater{} preprocess.py "repos 
...
$ pyhadoop commit_comments-dump.2015-01-29.json score classify.py reducer.py \
  ...
  commit_comments-dump.2015-01-29.json
\end{verbatim}
...
In order too count frequencies of scores within a specific group, use the 
following command:
...
\subsubsection{Update $\sim$/.bashrc}\label{app:update-bashrc}
...
troubles with installing or upgrading dependencies.
...
OpenBLAS. This BLAS implementation not only performs better than standard BLAS 
installations according to benchmarks at 
\url{http://stackoverflow.com/a/7645939}, but \textsc{SciPy} requires it 
because Lapack/BLAS are not installed on the DAS-3. Follow the instructions 
from step~2 onward from \url{http://stackoverflow.com/a/14391693}. Make sure to 
use \texttt{/home/\{username\}/.local} for the paths.
...
\subsubsection{Update $\sim$/.bashrc again}
...
be able to connect to other nodes via SSH. Although SSH access is possible, one 
must ensure that there are no interactive authentication prompts when MPI tries 
to open an SSH connection. This is complicated further by the fact that MPI 
might open SSH connections on other nodes as well.
...
This has to be done to make the virtual environment actually relocatable, 
otherwise it would be nonfunctional on the nodes, where we would still have 
Python 2.6.

Once everything is set up, check whether the Python scripts are distributed as 
follows: \texttt{pympi 8 mpi-test.py "" -{}-hostfile hosts -{}-map-by node}.
...
\subsubsection{Finding application logs}
